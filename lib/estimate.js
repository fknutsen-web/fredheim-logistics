...
// Three long-term partnership models (transactional single-shipment removed).
const ENGAGEMENTS = ['desk', 'program', 'advisory'];
...
export function computeEstimate(config = {}, R) {
  let engagement = config.engagement === 'recurring' ? 'program' : config.engagement;
  engagement = ENGAGEMENTS.includes(engagement) ? engagement : 'desk';
  ...
  if (engagement === 'program') {
    const perMT = +(R.recur.rate * complexity).toFixed(2);
    return { ...base, kind: 'perMT', label: 'Estimated Per-Tonne Management Fee',
             amount: perMT, display: cur + perMT.toFixed(2), per: 'per metric tonne · 12-month minimum' };
  }
  if (engagement === 'advisory') {
    const fee = round((R.advisory?.retainer || 9000) * 3 * complexity, 500);
    return { ...base, kind: 'project', label: 'Estimated Fixed-Fee Advisory Project',
             amount: fee, display: cur + fee.toLocaleString('en-US'), per: 'per project · defined scope & duration' };
  }
  // desk — outsourced commercial freight desk (monthly retainer or hybrid)
  const monthly = round((R.desk.retainer / 12) * complexity, 500);
  return { ...base, kind: 'monthly', label: 'Estimated Monthly Retainer',
           amount: monthly, display: cur + monthly.toLocaleString('en-US'),
           per: 'per month · retainer or hybrid · 12-month minimum' };
}
